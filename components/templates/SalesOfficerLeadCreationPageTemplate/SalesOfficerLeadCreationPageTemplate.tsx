...
const SalesOfficerLeadCreationPageTemplate = () => {
  ...
  const fetchLeads = async (page = 1) => {
    setIsLoading(true);
    try {
      const response = await soLeadService.getLeads(page, itemsPerPage, {
        searchTerm,
  ...
  const handleUpdateLead = async () => {
    ...
    try {
      ...
      await soLeadService.updateLead(editingLead._id, payload);
  ...
  const confirmAction = async () => {
    if (showConfirmModal.type === "delete" && showConfirmModal.leadId) {
      ...
      setTotalLeads((prev) => Math.max(0, prev - 1));
      try {
        await soLeadService.deleteLead(showConfirmModal.leadId!);
      } catch (error) {
      ...
      try {
        await soLeadService.updateLeadStatus(
          showConfirmModal.leadId!,
  ...
  const handleSaveRemarks = async () => {
    ...
    try {
      ...
      await soLeadService.updateLeadRemarks(remarksLead._id, remarksForApi);
...
export default SalesOfficerLeadCreationPageTemplate;
